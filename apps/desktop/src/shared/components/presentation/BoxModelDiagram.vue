...
import type { BoxModelDimensions, BoxModelRegion, StyleDeclarationEntry } from "./box-model.types";

const props = defineProps<{
  boxModel: BoxModelDimensions | null;
  computedStyles: StyleDeclarationEntry[];
}>();

const emit = defineEmits<{
  regionHover: [region: BoxModelRegion];
}>();
...
const hoveredRegion = ref<BoxModelRegion>(null);

function highlight(region: BoxModelRegion) {
  hoveredRegion.value = region;
  emit("regionHover", region);
}

function isRegion(r: BoxModelRegion) {
