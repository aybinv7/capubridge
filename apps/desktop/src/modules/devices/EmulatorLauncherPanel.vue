...
async function launch(avdName: string) {
  ...
  try {
    ...
    toast.success("Emulator launch requested", { description: `${avdName} will appear when ADB is ready.` });
    emit("launched");
  } catch (cause) {
...
<template>
  <div class="flex h-full flex-col overflow-hidden">
    <div class="flex shrink-0 items-start justify-between gap-4 border-b border-border/15 px-6 pb-4 pt-5">
      <div>
    ...
    <div class="flex-1 overflow-y-auto px-6 py-5">
      <div v-if="error" class="rounded-xl border border-error/25 bg-error/5 px-4 py-3 text-[11px] text-error">
        {{ error }}
      </div>

      <div v-else-if="loading && avds.length === 0" class="flex items-center gap-2 py-8 text-[11px] text-muted-foreground/35">
        <Loader2 :size="13" class="animate-spin" />
        Discovering installed AVDs…
      </div>

      <div v-else-if="avds.length" class="space-y-2">
        <div v-for="avd in avds" :key="avd.name" class="flex items-center gap-3 rounded-xl border border-border/20 bg-surface-1/50 px-4 py-3">
      ...
      <div v-else class="flex flex-col items-center gap-2 py-12 text-center">
        ...
        <p class="max-w-sm text-[11px] text-muted-foreground/25">Create an AVD in Android Studio, then refresh this list.</p>
      </div>
    </div>
  </div>
</template>
